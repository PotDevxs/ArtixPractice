...
final class KeyImpl implements Key {
   ...
   private static void checkError(String name, String namespace, String value, OptionalInt index) {
      if (index.isPresent()) {
         ...
         throw new InvalidKeyException(namespace, value, String.format("Non [a-z0-9_.-] character in %s of Key[%s] at index %d ('%s', bytes: %s)", name, asString(namespace, value), indexValue, character, Arrays.toString(String.valueOf(character).getBytes(StandardCharsets.UTF_8))));
      }
   }
