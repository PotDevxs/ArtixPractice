...
final class SerializerFactory implements TypeAdapterFactory {
   ...
   public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
      ...
      if (COMPONENT_TYPE.isAssignableFrom(rawType)) {
         ...
         TypeAdapter<T> adapter = (TypeAdapter<T>) ComponentSerializerImpl.create(gson);
         return adapter;
      } else if (KEY_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) KeySerializer.INSTANCE;
         return adapter;
      } else if (STYLE_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) StyleSerializer.create(this.legacyHoverSerializer, this.emitLegacyHover, gson);
         return adapter;
      } else if (CLICK_ACTION_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) ClickEventActionSerializer.INSTANCE;
         return adapter;
      } else if (HOVER_ACTION_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) HoverEventActionSerializer.INSTANCE;
         return adapter;
      } else if (SHOW_ITEM_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) ShowItemSerializer.create(gson);
         return adapter;
      } else if (SHOW_ENTITY_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) ShowEntitySerializer.create(gson);
         return adapter;
      } else if (COLOR_WRAPPER_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) TextColorWrapper.Serializer.INSTANCE;
         return adapter;
      } else if (COLOR_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) (this.downsampleColors ? TextColorSerializer.DOWNSAMPLE_COLOR : TextColorSerializer.INSTANCE);
         return adapter;
      } else if (TEXT_DECORATION_TYPE.isAssignableFrom(rawType)) {
         @SuppressWarnings("unchecked")
         TypeAdapter<T> adapter = (TypeAdapter<T>) TextDecorationSerializer.INSTANCE;
         return adapter;
      } else {
         if (BLOCK_NBT_POS_TYPE.isAssignableFrom(rawType)) {
            @SuppressWarnings("unchecked")
            TypeAdapter<T> adapter = (TypeAdapter<T>) BlockNBTComponentPosSerializer.INSTANCE;
            return adapter;
         }
         return null;
      }
