...
public final class TypeAdapters {
   ...
   public static <TT> TypeAdapterFactory newFactory(final Class<TT> unboxed, final Class<TT> boxed, final TypeAdapter<? super TT> typeAdapter) {
      return new TypeAdapterFactory() {
         public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
            ...
            return rawType != unboxed && rawType != boxed ? null : (TypeAdapter<T>) typeAdapter;
         }
   ...
   public static <TT> TypeAdapterFactory newFactoryForMultipleTypes(final Class<TT> base, final Class<? extends TT> sub, final TypeAdapter<? super TT> typeAdapter) {
      return new TypeAdapterFactory() {
         public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
            Class<? super T> rawType = typeToken.getRawType();
            return rawType != base && rawType != sub ? null : (TypeAdapter<T>) typeAdapter;
         }
   ...
   public static <T1> TypeAdapterFactory newTypeHierarchyFactory(final Class<T1> clazz, final TypeAdapter<T1> typeAdapter) {
      return new TypeAdapterFactory() {
         public <T2> TypeAdapter<T2> create(Gson gson, TypeToken<T2> typeToken) {
            ...
            return !clazz.isAssignableFrom(requestedType) ? null : (TypeAdapter<T2>) (new TypeAdapter<T1>() {
            ...
            if (value != null && !value.isJsonNull()) {
               if (value.isJsonPrimitive()) {
                  ...
                  if (value.isJsonArray()) {
                     out.beginArray();
                     elementIterator = value.getAsJsonArray().iterator();

                     while (elementIterator.hasNext()) {
                        JsonElement ex = (JsonElement) elementIterator.next();
                        this.write(out, ex);
                     }
                     ...
                     out.beginObject();
                     elementIterator = value.getAsJsonObject().entrySet().iterator();

                     while (elementIterator.hasNext()) {
                        Entry<String, JsonElement> e = (Entry) elementIterator.next();
   ...
   private static final class EnumTypeAdapter<T extends Enum<T>> extends TypeAdapter<T> {
         ...
         try {
            Field[] constantFields = (Field[])AccessController.doPrivileged(new PrivilegedAction<Field[]>() {
               public Field[] run() {
                  ...
                  Field[] constantFieldsArray = fields;
                  int fieldsLength = fields.length;

                  for (int i = 0; i < fieldsLength; ++i) {
                     Field f = constantFieldsArray[i];
                     if (f.isEnumConstant()) {
                        constantFieldsList.add(f);
                     }
                  }

                  Field[] result = (Field[])constantFieldsList.toArray(new Field[0]);
                  AccessibleObject.setAccessible(result, true);
                  return result;
               }
            });
            Field[] constantFieldsArray = constantFields;
            int constantFieldsLength = constantFields.length;

            for (int i = 0; i < constantFieldsLength; ++i) {
               Field constantField = constantFieldsArray[i];
               ...
               if (annotation != null) {
                  ...
                  int alternatesLength = alternates.length;

                  for (int j = 0; j < alternatesLength; ++j) {
                     String alternate = alternates[j];
      ...
      public T read(JsonReader in) throws IOException {
         if (in.peek() == JsonToken.NULL) {
            ...
            T constant = (T) this.nameToConstant.get(key);
            return constant == null ? (T) this.stringToConstant.get(key) : constant;
         }
      }
