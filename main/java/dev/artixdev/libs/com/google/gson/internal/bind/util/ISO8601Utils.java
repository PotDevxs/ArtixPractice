package dev.artixdev.libs.com.google.gson.internal.bind.util;
...
public class ISO8601Utils {
   ...
   public static Date parse(String date, ParsePosition pos) throws ParseException {
      ...
      try {
         ...
         offset += 4;
         int year = parseInt(date, startOffset, offset);
         ...
         startOffset = offset;
         offset += 2;
         int month = parseInt(date, startOffset, offset);
         if (checkOffset(date, offset, '-')) {
            ++offset;
         }
         ...
         startOffset = offset;
         offset += 2;
         int day = parseInt(date, startOffset, offset);
         int hour = 0;
         int minutes = 0;
         int seconds = 0;
         int milliseconds = 0;
         ...
         if (hasT) {
            ...
            startOffset = offset;
            offset += 2;
            hour = parseInt(date, startOffset, offset);
            if (checkOffset(date, offset, ':')) {
               ++offset;
            }
            ...
            startOffset = offset;
            offset += 2;
            minutes = parseInt(date, startOffset, offset);
            if (checkOffset(date, offset, ':')) {
               ++offset;
            }
            ...
            if (date.length() > offset) {
               ...
               if (c != 'Z' && c != '+' && c != '-') {
                  startOffset = offset;
                  offset += 2;
                  seconds = parseInt(date, startOffset, offset);
