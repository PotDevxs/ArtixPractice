...
import dev.artixdev.libs.com.google.gson.FieldNamingStrategy;
import dev.artixdev.libs.com.google.gson.Gson;
import dev.artixdev.libs.com.google.gson.JsonIOException;
...
import dev.artixdev.libs.com.google.gson.internal.ObjectConstructor;
import dev.artixdev.libs.com.google.gson.internal.Primitives;
import dev.artixdev.libs.com.google.gson.internal.ReflectionAccessFilterHelper;
import dev.artixdev.libs.com.google.gson.internal.reflect.ReflectionHelper;
...
public final class ReflectiveTypeAdapterFactory implements TypeAdapterFactory {
      ...
      if (!Object.class.isAssignableFrom(raw)) {
         ...
         if (filterResult == ReflectionAccessFilter.FilterResult.BLOCK_ALL) {
            ...
            if (ReflectionHelper.isRecord(raw)) {
               TypeAdapter<T> adapter = new ReflectiveTypeAdapterFactory.RecordAdapter(raw, this.getBoundFields(gson, type, raw, blockInaccessible, true), blockInaccessible);
               return adapter;
            } else {
   ...
   private ReflectiveTypeAdapterFactory.BoundField createBoundField(final Gson context, Field field, final Method accessor, String name, final TypeToken<?> fieldType, boolean serialize, boolean deserialize, final boolean blockInaccessible) {
      ...
      final boolean jsonAdapterPresent = annotation != null;
      final TypeAdapter<?> finalMapped = mapped;
      ...
      return new ReflectiveTypeAdapterFactory.BoundField(name, field, serialize, deserialize) {
            ...
            if (this.serialized) {
               ...
               if (fieldValue != source) {
                  ...
                  if (jsonAdapterPresent) {
                     ...
                     TypeAdapter<Object> adapter = (TypeAdapter<Object>)finalMapped;
                     t = adapter;
                  } else {
                     t = new TypeAdapterRuntimeTypeWrapper(context, finalMapped, fieldType.getType());
                  }
         ...
         void readIntoArray(JsonReader reader, int index, Object[] target) throws IOException, JsonParseException {
            Object fieldValue = finalMapped.read(reader);
   ...
   private Map<String, ReflectiveTypeAdapterFactory.BoundField> getBoundFields(Gson context, TypeToken<?> type, Class<?> raw, boolean blockInaccessible, boolean isRecord) {
         ...
         for(Class originalRaw = raw; raw != Object.class; raw = type.getRawType()) {
            ...
            for (int i = 0; i < fieldsLength; ++i) {
               ...
               if (serialize || deserialize) {
                  ...
                  for(int size = fieldNames.size(); j < size; ++j) {
                     String name = (String)fieldNames.get(j);
                     if (j != 0) {
                        serialize = false;
                     }

                     ReflectiveTypeAdapterFactory.BoundField boundField = this.createBoundField(context, field, accessor, name, TypeToken.get(fieldType), serialize, deserialize, blockInaccessible);
   ...
   public abstract static class Adapter<T, A> extends TypeAdapter<T> {
      ...
      public void write(JsonWriter out, T value) throws IOException {
         if (value == null) {
            ...
            try {
               Iterator<ReflectiveTypeAdapterFactory.BoundField> boundFieldIterator = this.boundFields.values().iterator();

               while (boundFieldIterator.hasNext()) {
                  ReflectiveTypeAdapterFactory.BoundField boundField = boundFieldIterator.next();
      ...
      abstract void readField(A accumulator, JsonReader reader, ReflectiveTypeAdapterFactory.BoundField field) throws IllegalAccessException, IOException;
