...
final class InstanceCreatorImpl<T> implements InstanceCreator<T> {
      ...
      if (creatorExecutable.getProperties().isEmpty()) {
         ...
         for(int i = 0; i < creatorExecutable.getProperties().size(); ++i) {
            if (creatorExecutable.getIdPropertyIndex() != null && creatorExecutable.getIdPropertyIndex() == i) {
               ...
               this.properties.put(((BsonProperty)creatorExecutable.getProperties().get(i)).value(), i);
            }
         }
   ...
   public <S> void set(S value, PropertyModel<S> propertyModel) {
      if (this.newInstance != null) {
         ...
         if (!this.properties.isEmpty()) {
            ...
            Integer index = (Integer)this.properties.get(propertyName);
            if (index != null) {
   ...
   public T getInstance() {
      if (this.newInstance == null) {
         try {
            ...
            throw new CodecConfigurationException(String.format("Could not construct new instance of: %s. Missing the following properties: %s", this.creatorExecutable.getType().getSimpleName(), this.properties.keySet()), ex);
         }
      }
   ...
   private void constructInstanceAndProcessCachedValues() {
      try {
         ...
         throw new CodecConfigurationException(ex.getMessage(), ex);
      }

      for (Entry<PropertyModel<?>, Object> entry : this.cachedValues.entrySet()) {
         this.setPropertyValue((PropertyModel) entry.getKey(), entry.getValue());
      }
   }

   @SuppressWarnings("unchecked")
   private <S> void setPropertyValue(PropertyModel<S> propertyModel, Object value) {
      this.set((S) value, propertyModel);
   }
}
