...
public class BsonDocumentCodec implements CollectibleCodec<BsonDocument> {
   ...
   public void encode(BsonWriter writer, BsonDocument value, EncoderContext encoderContext) {
      ...
      while(iterator.hasNext()) {
         Entry<String, BsonValue> entry = iterator.next();
         if (!this.skipField(encoderContext, (String)entry.getKey())) {
            writer.writeName((String)entry.getKey());
            this.writeValue(writer, encoderContext, (BsonValue)entry.getValue());
         }
      }
   ...
   public BsonDocument generateIdIfAbsentFromDocument(BsonDocument document) {
      if (!this.documentHasId(document)) {
         document.put((String)"_id", (BsonValue)(new BsonObjectId(new ObjectId())));
      }
   ...
   static {
      DEFAULT_BSON_TYPE_CODEC_MAP = new BsonTypeCodecMap(BsonValueCodecProvider.getBsonTypeClassMap(), DEFAULT_REGISTRY);
   }
}
