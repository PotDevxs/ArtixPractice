...
import dev.artixdev.libs.org.bson.io.BsonInput;
import dev.artixdev.libs.org.bson.io.BsonInputMark;
...
public class BsonBinaryReader extends AbstractBsonReader {
   ...
   public BsonBinaryReader(ByteBuffer byteBuffer) {
      this((BsonInput)(new ByteBufferBsonInput(new ByteBufNIO((ByteBuffer)Assertions.notNull("byteBuffer", byteBuffer)))));
   }
   ...
   public BsonBinaryReader(BsonInput bsonInput) {
      if (bsonInput == null) {
         ...
         this.setContext(new BsonBinaryReader.Context((BsonBinaryReader.Context)null, BsonContextType.TOP_LEVEL, 0, 0));
      }
   }
   ...
   public BsonType readBsonType() {
      ...
      } else if (this.getState() != AbstractBsonReader.State.INITIAL && this.getState() != AbstractBsonReader.State.DONE && this.getState() != AbstractBsonReader.State.SCOPE_DOCUMENT) {
   ...
   protected void doSkipValue() {
      if (this.isClosed()) {
         ...
         if (this.getState() != AbstractBsonReader.State.VALUE) {
            this.throwInvalidState("skipValue", new AbstractBsonReader.State[]{AbstractBsonReader.State.VALUE});
         }
   ...
   protected class Mark extends AbstractBsonReader.Mark {
      ...
      public void reset() {
         ...
         BsonBinaryReader.this.setContext(BsonBinaryReader.this.new Context((BsonBinaryReader.Context)this.getParentContext(), this.getContextType(), this.startPosition, this.size));
      }
   }
}
