...
class JsonScanner {
   ...
   JsonScanner(String json) {
      this((JsonBuffer)(new JsonStringBuffer(json)));
   }

   JsonScanner(Reader reader) {
      this((JsonBuffer)(new JsonStreamBuffer(reader)));
   }
   ...
   private static enum RegularExpressionState {
      ...
      // $FF: synthetic method
      private static JsonScanner.RegularExpressionState[] $values() {
         return new JsonScanner.RegularExpressionState[]{IN_PATTERN, IN_ESCAPE_SEQUENCE, IN_OPTIONS, DONE, INVALID};
      }
   }
   ...
   private static enum NumberState {
      ...
      // $FF: synthetic method
      private static JsonScanner.NumberState[] $values() {
         return new JsonScanner.NumberState[]{SAW_LEADING_MINUS, SAW_LEADING_ZERO, SAW_INTEGER_DIGITS, SAW_DECIMAL_POINT, SAW_FRACTION_DIGITS, SAW_EXPONENT_LETTER, SAW_EXPONENT_SIGN, SAW_EXPONENT_DIGITS, SAW_MINUS_I, DONE, INVALID};
      }
   }
}
