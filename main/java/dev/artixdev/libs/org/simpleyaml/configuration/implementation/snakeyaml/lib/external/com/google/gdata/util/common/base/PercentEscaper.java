package dev.artixdev.libs.org.simpleyaml.configuration.implementation.snakeyaml.lib.external.com.google.gdata.util.common.base;
...
public class PercentEscaper extends UnicodeEscaper {
   ...
   private static boolean[] createSafeOctets(String safeChars) {
      ...
      for (int i = 0; i < c; ++i) {
         char ch = safeCharArray[i];
         maxChar = Math.max(ch, maxChar);
      }
      ...
      int len = safeCharArray.length;
      for (int j = 0; j < len; ++j) {
         char ch = safeCharArray[j];
         octets[ch] = true;
      }
