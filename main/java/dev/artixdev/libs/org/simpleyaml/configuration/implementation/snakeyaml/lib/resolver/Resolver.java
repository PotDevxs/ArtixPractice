...
public class Resolver {
   ...
   public void addImplicitResolver(Tag tag, Pattern regexp, String first, int limit) {
      if (first == null) {
         List<ResolverTuple> curr = (List)this.yamlImplicitResolvers.get(null);
         if (curr == null) {
            curr = new ArrayList();
            this.yamlImplicitResolvers.put(null, curr);
         }

         ((List)curr).add(new ResolverTuple(tag, regexp, limit));
      } else {
   ...
   public Tag resolve(NodeId kind, String value, boolean implicit) {
      if (kind == NodeId.scalar && implicit) {
         ...
         Iterator<ResolverTuple> resolverIterator;
         ResolverTuple v;
         Tag tag;
         Pattern regexp;
         if (resolvers != null) {
         ...
         if (this.yamlImplicitResolvers.containsKey(null)) {
            resolverIterator = ((List<ResolverTuple>)this.yamlImplicitResolvers.get(null)).iterator();
