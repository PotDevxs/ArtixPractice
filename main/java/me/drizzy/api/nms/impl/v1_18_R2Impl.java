...
public class v1_18_R2Impl implements INMSImpl {
   ...
   public boolean isViewingInventory(Player player) {
      CraftPlayer craftPlayer = (CraftPlayer)player;
      return craftPlayer.getHandle().bV.j != 0;
   }

   public void sendEatingAnimation(Player player) {
      Entity entityPlayer = ((CraftPlayer)player).getHandle();
      ChunkProviderServer chunkproviderserver = ((WorldServer)entityPlayer.s).k();
   ...
   public void setJumping(Player player) {
      ...
      entityPlayer.q(true);
      if (!entityPlayer.aQ() && !entityPlayer.bc()) {
         if (entityPlayer.aw()) {
            entityPlayer.eA();
         }
      } else {
         entityPlayer.g(entityPlayer.da().d(0.0D, 0.03999999910593033D, 0.0D));
      }
