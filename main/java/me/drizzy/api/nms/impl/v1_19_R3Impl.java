...
public class v1_19_R3Impl implements INMSImpl {
   ...
   public boolean isViewingInventory(Player player) {
      CraftPlayer craftPlayer = (CraftPlayer)player;
      return craftPlayer.getHandle().bP.j != 0;
   }

   public void sendEatingAnimation(Player player) {
      Entity entityPlayer = ((CraftPlayer)player).getHandle();
      ChunkProviderServer chunkproviderserver = ((WorldServer)entityPlayer.H).k();
   ...
   public void setJumping(Player player) {
      ...
      entityPlayer.r(true);
      if (!entityPlayer.aT() && !entityPlayer.bg()) {
         if (player.isOnGround()) {
            entityPlayer.eS();
         }
      } else {
         entityPlayer.f(entityPlayer.dj().b(0.0D, 0.03999999910593033D, 0.0D));
      }
