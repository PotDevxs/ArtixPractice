...
public class v1_19_R2Impl implements INMSImpl {
   ...
   public boolean isViewingInventory(Player player) {
      try {
         Object entityPlayer = getHandle(player);
         Field containerField = findField(entityPlayer.getClass(), "bU");
   ...
   public void sendEatingAnimation(Player player) {
      try {
         Object entityPlayer = getHandle(player);
         Method cH = entityPlayer.getClass().getMethod("cH");
         Object world = cH.invoke(entityPlayer);
   ...
   public void setJumping(Player player) {
      try {
         ...
         Method aV = entityPlayer.getClass().getMethod("aV");
         Method bi = entityPlayer.getClass().getMethod("bi");
         if (!(Boolean) aV.invoke(entityPlayer) && !(Boolean) bi.invoke(entityPlayer)) {
            if (player.isOnGround()) {
               Method eN = entityPlayer.getClass().getMethod("eN");
               eN.invoke(entityPlayer);
            }
         } else {
            Method di = entityPlayer.getClass().getMethod("di");
            Object vec = di.invoke(entityPlayer);
   ...
   public v1_19_R2Impl(JavaPlugin plugin) {
      this.plugin = plugin;
   }
}
