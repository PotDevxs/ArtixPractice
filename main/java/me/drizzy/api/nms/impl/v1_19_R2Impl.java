...
public class v1_19_R2Impl implements INMSImpl {
   ...
   public boolean isViewingInventory(Player player) {
      CraftPlayer craftPlayer = (CraftPlayer)player;
      return craftPlayer.getHandle().bU.j != 0;
   }

   public void sendEatingAnimation(Player player) {
      Entity entityPlayer = ((CraftPlayer)player).getHandle();
      ChunkProviderServer chunkproviderserver = ((WorldServer)entityPlayer.cH()).k();
   ...
   public void setJumping(Player player) {
      ...
      if (!entityPlayer.aV() && !entityPlayer.bi()) {
         ...
         entityPlayer.f(entityPlayer.di().b(0.0D, 0.03999999910593033D, 0.0D));
      }
