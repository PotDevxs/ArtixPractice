...
public class v1_16_R3PacketListener extends AbstractPacketListener {
      ...
      if (event.getPlayer() != null) {
         if (event.getPacket() != null) {
            ...
            if (type == Server.WORLD_EVENT) {
               ...
               entityID = (Integer)event.getPacket().getIntegers().readSafely(0);
               net.minecraft.server.v1_16_R3.Entity nmsEntity = ((CraftWorld)receiver.getWorld()).getHandle().getEntity(entityID);
               if (nmsEntity == null) {
                  return;
               }
   ...
   public Player getPlayerWhoDropped(Item item) {
      UUID thrower = ((EntityItem)((CraftItem)item).getHandle()).getThrower();
